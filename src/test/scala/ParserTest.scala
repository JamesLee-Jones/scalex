...
import lexer.{Alt, Ch, Emp, ParseError, Parser, RegEx, Scanner, Sequence, SourcePosition, Star}
...
class ParserFlatSpec extends AnyFlatSpec {
  ...
  "An sequence regex" should "return a Seq" in {
    assert(Parser.parseRegex(Scanner.scan("gb")) == Right(Sequence(Ch('g'), Ch('b'))))
    assert(
      Parser.parseRegex(Scanner.scan("gb64")) == Right(Sequence(
        Ch('g'),
        Ch('b'),
        Ch('6'),
        Ch('4')
  ...
  "An alternation" should "return an alternation with the regexes either side" in {
    assert(Parser.parseRegex(Scanner.scan("a|b")) == Right(Alt(Ch('a'), Ch('b'))))
    assert(
      Parser.parseRegex(Scanner.scan("a1|b2")) == Right(Alt(
        Sequence(Ch('a'), Ch('1')),
        Sequence(Ch('b'), Ch('2'))
      ))
    )
  ...
  "Matching brackets" should "parse correctly" in {
    assert(Parser.parseRegex(Scanner.scan("()")) == Right(Emp))
    assert(Parser.parseRegex(Scanner.scan("(a)")) == Right(Ch('a')))
    assert(Parser.parseRegex(Scanner.scan("(ab)")) == Right(Sequence(Ch('a'), Ch('b'))))
    assert(
      Parser.parseRegex(Scanner.scan("(a1)|(b2)")) == Right(Alt(
        Sequence(Ch('a'), Ch('1')),
        Sequence(Ch('b'), Ch('2'))
    ...
    assert(
      Parser.parseRegex(Scanner.scan("(a1|b)2")) == Right(Sequence(
        Alt(Sequence(Ch('a'), Ch('1')), Ch('b')),
        Ch('2')
      ))
    )
    assert(
      Parser.parseRegex(Scanner.scan("a(1|b)2")) == Right(Sequence(
        Ch('a'),
        Alt(Ch('1'), Ch('b')),
        Ch('2')
      ))
    ...
    assert(
      Parser.parseRegex(Scanner.scan("a(1|b2)")) == Right(Sequence(
        Ch('a'),
  ...
  "Closure" should "bind correctly" in {
    ...
    assert(
      Parser.parseRegex(Scanner.scan("ab*")) == Right(Sequence(Ch('a'), Star(Ch('b'))))
    )
    assert(
      Parser.parseRegex(Scanner.scan("a(1|b)*2")) == Right(Sequence(
        Ch('a'),
  ...
  "Non matching brackets" should "return an error" in {
    val rightError = (line: Int, col: Int) => Left(ParseError("Left bracket does not have a matching right bracket.", SourcePosition(line, col)))
    val leftError = (line: Int, col: Int) => Left(ParseError("Right bracket does not have a matching left bracket.", SourcePosition(line, col)))
    val expected = (line: Int, col: Int) => Left(ParseError("", SourcePosition(line, col)))
  ...
  "An alternation without two arguments" should "return an error" in {
    val rightError = (line: Int, col: Int) => Left(ParseError("The right-hand side of an alternation must be an expression.", SourcePosition(line, col)))
    val leftError = (line: Int, col: Int) => Left(ParseError(s"The left-hand side of an alternation must be an expression.", SourcePosition(line, col)))
